...
function openEditRoomModal(roomId) {
  ...
  fetch(`/rooms/${roomId}`, {
    ...
    .catch((error) => {
      showNotification(
        "error",
        "Có lỗi xảy ra khi tải thông tin phòng: " + error.message
      );
      modal.hide();
    });
}
  ...
  fetch(`/rooms/${roomId}`, {
    ...
    .then((data) => {
      if (data.success && data.room) {
        ...
        // Show error template
        let errorTemplate = document.getElementById(
          "roomDetailErrorTemplate"
        ).innerHTML;
        const errorMessage =
          data.message || "Không tìm thấy thông tin phòng hoặc dữ liệu trống";
    ...
    .catch((error) => {
      ...
      errorTemplate = errorTemplate.replaceAll(
        "{{message}}",
